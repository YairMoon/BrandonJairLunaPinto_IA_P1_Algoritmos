...
def policy_iteration(states, actions, transition_model, reward, gamma=0.9, epsilon=0.01):
    # Inicializamos la política (al azar) y los valores de los estados
    policy = {s: actions[0] for s in states}  # Política inicial
...
# El algoritmo de Iteración de Políticas es un método para encontrar la política óptima en un Proceso de Decisión de Markov (MDP).
# En este ejemplo, definimos un MDP simple con dos estados y dos acciones posibles.